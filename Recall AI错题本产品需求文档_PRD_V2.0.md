...
本项目先用 3 个完整工作日完成受控试用版，验证核心交互是否成立；通过验证后，再进入本文其余章节定义的完整 MVP。试用版是完整 MVP 的前置实验，不替代、不降低完整 MVP 的质量、安全和合规门槛。

### 0.1 两阶段目标

| 阶段 | 周期 | 使用范围 | 要验证什么 | 明确不验证什么 |
|---|---:|---|---|---|
| T0 受控试用版 | 3 个完整工作日 | 3-5 名已知情的成年内部测试者 | 是否愿意上传、确认、记录个人理解并再次复习 | 规模化、未成年人合规、7 日留存、OCR 精度、付费和变式迁移 |
| M1 完整 MVP | 试用版通过后约 11-14 周 | 5 人种子测试，再扩大到 20-50 人邀请制试点 | 录入质量、复习留存、变式迁移、成本、数据权利和付费信号 | 多学科、教师端、课程和社区 |

### 0.2 三天试用版核心链路

```mermaid
flowchart LR
    A[受控访问与简明告知] --> B[上传一张清晰单题图片]
    B --> C[多模态模型返回结构化结果]
    C --> D[用户编辑并确认]
    D --> E[保存到本机错题库]
    E --> F[记录个人解题便签]
...
即使在试用版中，上传成功也不等于正式入库；用户确认仍是不可绕过的产品原则。标准解析默认折叠，个人便签优先展示。

### 0.3 三天试用版范围

#### 必须实现

- 移动端优先的受控访问页和 AI 图片处理简明告知；
- 单张、单题、清晰印刷体数学图片上传；
- 一个多模态模型直接输出题干、学生答案、知识点、错因、提示、关键步骤和完整解析的严格 JSON；
- 原图对照、字段编辑和用户确认；
- 浏览器本地错题库、错题详情和个人解题便签；
- 立即复习与次日到期复习、作答、自评和下次复习时间；
- 单题删除、清空本机数据、失败重试和基础移动端适配。
- AI 识图失败时允许保存为传统错题：保留原图和用户便签，但不生成 AI 分析，不进入 AI 复习区。

#### 不进入试用版

- 一图多题切分、独立 OCR、复杂公式/手写/几何识别；
- 正式账号、跨设备同步、监护人同意验证和未成年人真实试用；
- PostgreSQL、Redis、对象存储、异步 Worker 和完整 Model Gateway；
- 变式题、PDF、学习统计、搜索筛选、批量操作、支付和运营后台；
- 生产级并发、完整审计、公开上线和 20-50 人试点。

这些能力继续由本文后续完整 MVP 章节定义，并在试用版决策通过后实施。

### 0.4 试用版验证与决策门槛

试用结束后必须形成一页验证记录，至少包含任务完成情况、观察记录、失败截图、用户原话、模型错误和下一步决定。

进入完整 MVP 的最低条件：

- 至少 3 名测试者在不依赖开发者代操作的情况下完成“上传 -> 确认 -> 入库 -> 便签 -> 复习”；
- 大多数测试者能正确理解这是错题复习工具，而不是拍照搜答案入口；
- 测试者能够发现并修改错误识别，不误认为 AI 内容已经自动确认；
- 至少 2 名测试者愿意在次日再次打开并完成一题复习；
- 没有导致图片、题目或复习记录不可恢复丢失的阻断错误。

决策只有三种：`继续完整 MVP`、`调整核心体验后再测`、`停止投入并复盘`。不得因为代码已经完成而自动进入完整 MVP。
...
需要特别承认一个产品事实：即使完全移除 AI，用户仍然可以用图片和文字做出一个电子错题本。因此，“上传图片后自动整理”只能证明产品更方便，不能单独证明产品必须使用 AI。

Recall 的 AI 差异应落在复习之后，而不是只落在上传之后：

- 上传阶段：AI 负责识别和整理，降低建立错题资产的成本；
- 理解阶段：AI 提供可核对的提示、可能卡点和关键步骤，但不替用户确认分类；
- 复习阶段：AI 结合用户本次作答、复习感受和历史便签，生成一次短复盘，帮助用户知道下一次先检查什么；
- 长期阶段：AI 只在有足够行为证据时总结反复出现的错误模式，不凭一次上传给用户贴标签。

因此，产品对外不应只说“AI 错题本”，而应让用户感知“这是一本会根据你的复习表现帮你复盘的错题本”。AI 复盘失败时，错题保存和复习排期仍必须正常完成。

当上传识别失败时，产品应承认传统错题本路径的价值：允许用户直接保存原图并记录自己的便签。这类记录不应伪装成 AI 已分析错题，也不应进入 AI 复习区；后续用户可以重新裁剪或重新上传，再创建 AI 错题。
